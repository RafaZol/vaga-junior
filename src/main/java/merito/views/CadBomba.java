...
public class CadBomba extends javax.swing.JFrame {
    ...
    private List<BombaModel> bombas;
     ...
     * Creates new form CadBomba
          ...
          */
         public CadBomba() throws ClassNotFoundException {
        initComponents();
    ...
    protected void tbConsultaLinhaSelecionada(JTable tb){
        try{
            if(tb.getSelectedRow() != -1 && !inicializando){
                ...
                for(int i = 0; i < produtos.size(); i++){
                    ProdutoModel get = produtos.get(i);
    ...
    private void salvar(){
        if(cbCombustivel.getSelectedIndex() > -1){
            try {
                ...
                bomba.setCombustivel(produtos.get(cbCombustivel.getSelectedIndex()).getId());
                if(tfId.getText().trim().isEmpty()){
                    bomba.setId(UtilsDao.gerarId("bb_id","tb_bomba"));
                    produtoDao.cadastrarBomba(bomba);
                    limparCampos();
                } else {
                    bomba.setId(Long.parseLong(tfId.getText()));
                    produtoDao.alterarBomba(bomba);
                    limparCampos();
                }
                listar();
            } catch (Exception ex){
    ...
    private void excluir() throws ClassNotFoundException{
        if(tbConsulta.getSelectedRow() != -1){
            ...
            produtoDao.excluirBomba(bombas.get(ls).getId());
        }
        listar();
    }
    ...
    private void mostrar(List<BombaModel> lista){
        inicializando = true;
