...
public class CadPessoas extends javax.swing.JFrame {
    ...
    private void salvar(){
        if(verificaCampos()){
            try{
                ...
                //int tipo;
                pessoaModel.setNome(tfNome.getText());
                pessoaModel.setIdade(Integer.parseInt(tfIdade.getText()));
                pessoaModel.setDoc(Long.parseLong(tfDocumento.getText()));
                pessoaModel.setEmail(tfEmail.getText());
                pessoaModel.setTelefone(Long.parseLong(tfTelefone.getText()));
                pessoaModel.setTipo(tipoDocumento());                                        
                if(tfId.getText().trim().isEmpty()){
                    pessoaModel.setId(UtilsDao.gerarId("tb_pessoa_id","tb_pessoas"));
                    pessoaDao.cadastrar(pessoaModel);
                    limparCampos();
                } else {
                    pessoaModel.setId(Long.parseLong(tfId.getText()));
                    pessoaDao.alterar(pessoaModel);
                    limparCampos();
                }
    ...
    private int tipoDocumento(){
        int tipo = 0;
        switch (tfDocumento.getText().trim().length()) {
            case 11:
                tipo = 0;
                break;
            case 14:
    ...
    private void excluir() throws ClassNotFoundException{
        if(tbConsultas.getSelectedRow() != -1){
            ...
            pessoaDao.excluir(pessoas.get(ls).getId());
        }
        listar();
    }
    ...
    private void mostrar (List<PessoaModel> list){
        inicializando = true;
    ...
    protected void tbConsultaLinhaSelecionada(JTable tb){
        try {
            if(tb.getSelectedRow() != -1 && !inicializando){
                ...
                int tipo = 0;
                switch (pessoas.get(ls).getTipo()) {
                    case 0:
                       tfTipo.setText("P. Física");
                        break;
                    case 1:
                        tfTipo.setText("P. Jurídica");
                        break;
                    default:
